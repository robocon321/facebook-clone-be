...
public class CommentPostService {
	...
	public CommentPostResponse createComment(CommentPostRequest request, Integer accountId) {
		...
		CommentPostResponse response = mapEntityToDTO(newEntity);
		return response;
	}
	...
	public List<CommentPostResponse> getAllCommentByPost(Integer postId) {
		...
		List<CommentPostResponse> responses = entities.stream().map(item -> mapEntityToDTO(item)).toList();
		return responses;
	}
...
//			CommentPostResponse parentCommentPostResponse =  new CommentPostResponse();
//			CommentPostEntity parent = entity.getParent();
//			BeanUtils.copyProperties(parent, parentCommentPostResponse);
